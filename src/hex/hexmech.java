...
*********************************************************/
	public static Polygon hex (int x0, int y0) {
		...
		int y = (y0 * h + (x0%2) * h/2) + BORDERS ;
		int x = (x0 * (s+t)) + BORDERS; // + (XYVertex ? t : 0); //Fix added for XYVertex = true. 
...
*****************************************************************************/
	public static void fillHex(int i, int j, int res, int num, Graphics2D g2) {
		
		int y = j * h + (i%2) * h/2;
		int x = i * (s+t); // + (XYVertex ? t : 0); //Fix added for XYVertex = true. 
		
		g2.setColor(res >= 0 ? BoardDisplay.COLORARRAY[res] : BoardDisplay.EMPTY);
