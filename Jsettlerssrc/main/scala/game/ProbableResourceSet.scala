...
case class ProbableResourceSet(known: ResourceSet, unknown: UnknownResources) extends ResourceSet{
...
//  }
  ...
  override def toString: String = ProbableResourceSet.resTypes.filter(getTotalProbableAmount(_) > 0).map { res =>
    s"${getProbabilityOfResourceInHand(res)}"
  }.mkString("")

//  def map(resourcesType: Int)(fn: Int => Double): ProbableResourceSet = copy(
//    cl = if(resourcesType == SOCResourceConstants.CLAY) fn(resourcesType == SOCResourceConstants.CLAY) else cl
//  )
...
object ProbableResourceSet {
  ...
  def calculateProbableResources(transactions: List[SOCTransactions]): Map[Int, ProbableResourceSet] = {
    ...
    val nonStealTransactions = transactionsWithIndex.filterNot(_._1.isInstanceOf[Rob])
    val stealTransactions = transactionsWithIndex.filter(_._1.isInstanceOf[Rob])
    ...
    nonStealTransactions.foreach {
      case (Gain(player, set), i) => nonStealResources(player).add(set)
...
case class Rob(robber: Int, victim: Int, resourceSet: Option[SOCResourceSet]) extends SOCTransactions
